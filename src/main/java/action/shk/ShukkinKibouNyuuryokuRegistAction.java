...
public class ShukkinKibouNyuuryokuRegistAction extends ShukkinKibouAbstractAction{
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        // 登録・更新処理
        shukkinKibouLogic.registTsukibetsuShift(tsukibetsuShiftDtoListList, loginUserDto);

        // シフトIDを取得する
        //getTsukibetsuShiftDtoMapの引数をtrue→falseに変更
        ...
        for (TsukibetsuShiftNyuuryokuBean tsukibetsuShiftBean : tsukibetsuShiftBeanList) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (methods[i].getName().startsWith("getShiftId") && index < listSize) {
                    ...
                    String shainId = (String) methods[i].invoke(tsukibetsuShiftBean);

                    if (CommonConstant.BLANK_ID.equals(shainId)) {
                        // 空白が選択されている場合
                    	shainId = null;
                    }

                    tsukibetsuShiftDto.setShiftId(shainId);
